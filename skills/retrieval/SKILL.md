---
name: retrieval
description: 'Answer questions from live data through a running skardi-server using the skardi CLI. Discover at runtime what sources and named pipelines the server has (and table schemas where the deployment exposes them), run the question through any semantic search surface first (search-hybrid and friends), then write read-only SQL against exact qualified table names for the precise answer, check the truncated flag before trusting counts, and report with the query attached. Use whenever the user wants to query, explore, or answer questions from data a skardi-server already serves — Postgres, MySQL, SQLite, MongoDB, files, or SaaS sources such as GitHub and Slack registered in its ctx — or asks what data is available. Trigger on: query our database, ask our data a question, what tables do we have, run SQL through skardi, use the skardi CLI, how many orders/users/events, answer from the warehouse, which PRs are open or still unreviewed, who reviewed or commented on an issue, how many issues we closed last month, release, commit and workflow-run history, Slack conversations and files. Reach for this rather than gh or another vendor CLI whenever the question covers more than one record, needs history or aggregation, or joins a SaaS source with another source; a single known-record lookup through the vendor CLI is still fine. Requires a reachable skardi-server; this skill does not build search indexes (auto-context does that), does not write data (the job path does that), and does not set up or start servers.'
---
...
- **If a table came back with an empty column list**, the deployment does not
  ...
  # build without --task (probe: skardi query --help): drop that one flag:
  # skardi query --purpose "peek at orders to learn its columns" --session-id "$SKARDI_SESSION" \
  #   -e "SELECT * FROM shop.main.orders LIMIT 5" --table
  # v0.5.0 build (no --purpose/--session-id/--task): drop all three:
  # skardi query -e "SELECT * FROM shop.main.orders LIMIT 5" --table
  ```

  (`$SKARDI_SESSION` is the task's session id and the `--task` value is the
  task line; step 3 explains all three. Settle both before the first query,
  peeks included.)
