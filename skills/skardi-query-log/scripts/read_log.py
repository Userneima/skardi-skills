...
def kind_clause(con, kind):
    ...
    Matched case-insensitively, because the column carries no `COLLATE NOCASE`
    and the casing changed under it (SkardiLabs/skardi#219): servers built
    before it wrote the `Debug` form of the enum — `Query`, `Other` — and
    servers after it write `query`, `other`. Both casings sit in ledgers out
    there, and the migration that rewrites the old rows only runs when a
    current server opens the file. Matching `= 'query'` on a pre-#219 ledger
    returns zero rows and says nothing about it.
    """
    if kind == "all" or not has_statement_kind(con):
        return "", []
    return "LOWER(statement_kind) = ?", [kind]


def existing_kinds(con):
    """Which statement_kind values this ledger actually holds, verbatim (casing included)."""
    if not has_statement_kind(con):
        return []
    return [(r["k"], r["n"]) for r in con.execute(
        "SELECT statement_kind k, COUNT(*) n FROM query_audit"
        " GROUP BY statement_kind ORDER BY n DESC")]
...
def overview(con, kind):
    ...
    if not r["n"]:
        ...
        kinds = existing_kinds(con)
        if kinds:
            print("statement_kind values actually in this ledger: "
                  + ", ".join(f"{k} ({n})" for k, n in kinds) + ".")
        elif kind != "all" and has_statement_kind(con):
    ...
    breakdown = existing_kinds(con)
    if len(breakdown) > 1:
        print("whole ledger: " + ", ".join(f"{n} {k}" for k, n in breakdown))
